				  ...
				  log.Printf("delay detected: line %s between %s→%s (+%ds, %v)",
				      event.LineID, event.FromStation, event.ToStation,
					      event.DelaySecs, event.Severity)
						    ...
						    // Minor delays: only bust ETA cache for affected stations
							  // Major delays: bust all route caches touching this line
							...
							func (p *ETAWorkerPool) bustStationETAs(ctx context.Context, event DelayEvent) {
							  // Walk downstream stations from the delay point
										    ...
										    if len(keys) > 0 {
											    if err := p.rdb.Del(ctx, keys...).Err(); err != nil {
												      log.Printf("bust station ETAs: %v", err)
															...
															func (p *ETAWorkerPool) bustSegmentRoutes(ctx context.Context, event DelayEvent) {
															  // Route cache keys are: route:{fromStn}:{toStn}:{pref}
															    // We need to find all routes that include the delayed segment.
																  // Strategy: scan for route:* and check metadata field "lines"
																    // For performance, we maintain a secondary index:
																	  //   "routeidx:line:{lineId}" → set of route cache keys
																		    ...
																		    keys, err := p.rdb.SMembers(ctx, idxKey).Result()
																			  if err != nil || len(keys) == 0 {
																			      return
																					  ...
																					  // Filter to routes whose station list includes the delayed segment
																																					  ...
																																					  all := make([]string, 0, len(keys)*2)
																																					    for _, k := range keys {
																																						    all = append(all, k, k+":meta")
																																								...
																																								func (p *ETAWorkerPool) updateLineStatus(ctx context.Context, event DelayEvent) {
																																													    ...
																																													    "to_station", event.ToStation,
																																														    "since", event.DetectedAt.Format(time.RFC3339),
																																															    ...
																																															    p.rdb.Expire(ctx, key, 30*time.Minute)
																																																  ...
																																																  // Notify WebSocket subscribers watching this line
																																																		    ...
																																																		    "line_id":    event.LineID,
																																																			    "delay_secs": event.DelaySecs,
																																																				    "cause":      event.Cause,
																																																					    ...
																																																					    p.nc.Publish(fmt.Sprintf(SubjLineStatus, event.LineID), update)
																																																						}